...
* HOLD button down stars "CLEAR MEMORY" option. ALL LEDs ON then each goes out in sequence. 
...
* Removal of the HUB controller lid will be neccessary to allow access to BOTH buttons and the FTDI interface if needed.
* STEPS-BY-STEP:
...
* 3..Select this port.
* 4..To uplaod a sketch hold down the PAIR button while pressing the RESET button.
...
#define PROG_ID "ESP32-NOW_HUB_R1_STATS V1.2L"  // MKI PCB - NO ISOLATOR - Use analysis of antenna inpt to determine live wire OFF / HOT. Mk1 PCB LED values!
...
//   J: Fix EEPROM wite issue EEPROM_REFVAL_ADDR is an INT - allow 2 bytes!!! EEPROM_SIZE +4 -> +8  SENSOR_DIFF 100 -> 250
//   K: Add newline to "count = %i" in btnDown Fn. Add volatile int lastMsgStatus declaration. L: Variance 250 -> 600
...
#define EEPROM_SIZE (MAC_ARRAY_SIZE + 8)          // Allow for MAC array, NUMPAIRED cold start flag and SERIAL_MODE_FLAG. And Live off ref value!
...
#define EEPROM_REFVAL_ADDR 2      // THIS IS AN INT SO allow TWO BYTES!!!
#define EEPROM_MAC_ARRAY_ADDR 6   // Start address of MAC array.
...
#define SENSOR_DIFF 600             // ***ADJUST*** Analogue value variance from ref indicating live is HOT!
...
// USED IN INTERRUPTS...
volatile bool btnPressed = false;        // Flag set by interrupt code on button pressed.
volatile int lastMsgStatus = 0;          // Set by calling & callback fn. 0 = PENDING, 1 = GOOD, 2 = BAD.
...
//--------------------------------------------------------------------------------------------------------
  if (SERIALPRINT) Serial.printf("count = %i\n", count);
